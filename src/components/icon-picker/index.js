...
const NativeIconPicker = ({
    ...
    return (
        <>
            ...
            />
            <BaseControl id="marks-icon-settings" label={label} __nextHasNoMarginBottom>
                <Dropdown
                    popoverProps={{
                    ...
                    className="marks-icon-settings"
                    renderToggle={({ isOpen, onToggle, onClose }) => (
                        <div className="marks-icon-settings__dropdown">
                            <Button
                                onClick={onToggle}
                                aria-expanded={isOpen}
                                className="marks-icon-settings__dropdown-toggle"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            >
                                {!!(iconName || customSvgCode) ? (
                                    <span className="marks-icon-settings__indicator">
                                        <RenderIcon customSvgCode={customSvgCode} iconName={iconName} size={20} />
                                    </span>
                                ) : (
                                    <span className="marks-icon-settings__indicator disabled"></span>
                                )}

                                <span className="marks-icon-settings__dropdown-label">
                                    {__(iconName || (customSvgCode ? 'Custom SVG' : 'Select Icon', 'marks'))}
                                </span>
                            </Button>
                            <Button
                                label={__('Browse library', 'marks')}
                                onClick={() => {
                                    onClose();
                                    openModal('library');
                                }}
                                ...
                                size="small"
                                icon={folderOpen}
                                className="marks-icon-settings__dropdown-more"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            />
                        </div>
