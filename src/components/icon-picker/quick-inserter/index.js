...
export default function QuickInserter({ setIcon, onClose, openModal, withCustomSvg = true }) {
    ...
    const searchResults = (
        <div className="marks-icon-settings__icons">
            {shownIcons.map(icon => {
                ...
                return (
                    <Button
                        key={`icon-${icon.name}`}
                        label={__(icon.title)}
                        className={`marks-icon-settings__icons-button`}
                        onClick={() => {
                            setIcon(icon);
                            onClose();
                        }}
    ...
    return (
        <div className="marks-icon-settings__quick-inserter" style={{ width: '280px', padding: '8px' }}>
            <SearchControl
            ...
            />
            <div className="marks-icon-settings__quick-inserter-results">
