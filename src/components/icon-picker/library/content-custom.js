...
export const ContentCustom = ({
    ...
    const renderCurrentIcon = (size, customCode, customStroke) => {
        ...
        if (svgCode) {
            ...
            return (
                <div
                    className="marks-custom-svg-container"
    ...
    return (
        <div className="marks-modal__custom-svg">
            <div className="marks-modal__custom-svg-editor">
                <TextareaControl
                    label={__('Custom SVG code', 'marks')}
                    value={tempCustomSvgCode}
                    onChange={setTempCustomSvgCode}
                    help={__('Paste your custom SVG code here. It will override the selected icon.', 'marks')}
                    rows={15}
                />
            </div>

            <div className="marks-modal__custom-svg-preview">
                <h3>{__('Preview', 'marks')}</h3>
                <div className="marks-icon-preview-container">
                    {tempCustomSvgCode ? (
                        renderCurrentIcon(previewIconSize, tempCustomSvgCode, previewStrokeWidth)
                    ) : (
                        <div className="marks-empty-preview">{__('Enter SVG code to see preview', 'marks')}</div>
                    )}
                </div>

                <div className="marks-modal__custom-svg-controls">
                    <RangeControl
                ...
                <div className="marks-modal__custom-svg-buttons">
                    <Flex>
                        <Button
                            __next40pxDefaultSize
                            variant="secondary"
