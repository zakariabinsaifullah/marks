...
const Edit = props => {
    ...
    const innerBlocksProps = useInnerBlocksProps(
        {
            className: classNames('marks-editor-slides', {
                [`columns-${columns[resMode]}`]: columns[resMode],
                [`gap-${gaps[resMode]}`]: gaps[resMode]
            })
        },
        {
            allowedBlocks: ['marks/slide'],
            template: [['marks/slide'], ['marks/slide']],
            templateLock: false
        }
    );
        ...
        <>
            <BlockControls>
                <ToolbarGroup>
                    <ToolbarButton
                        icon="insert"
                        label={__('Add Slide', 'marks')}
                        onClick={() => {
                            const innerBlocks = wp.data.select('core/block-editor').getBlocks(clientId);
                            const newBlock = wp.blocks.createBlock('marks/slide');
