...
export default function Edit(props) {
    ...
    return (
        <>
            <BlockControls group="block">
                ...
                {isEditingURL && (
                    <Popover
                        ...
                        focusOnMount={true}
                        offset={12}
                        className="marks-icon__link-popover"
                        variant="alternate"
                    >
                        <LinkControl
                            value={{
            ...
            <InspectorControls>
                <PanelBody title={__('Settings', 'marks')}>
                    <NativeToggleControl
                        label={__('Add List Title', 'marks')}
                        checked={showTitle}
                    ...
                    />
                    <NativeResponsiveControl label={__('Icon Size (px)', 'marks')} props={props}>
                        <RangeControl
                            value={sizes[resMode]}
                ...
                {showTitle && (
                    <PanelBody title={__('List Title', 'marks')} initialOpen={false}>
                        <NativeUnitControl
                            label={__('Gap ', 'marks')}
                            value={listGap}
                        ...
                        {showTitle && (
                            <>
                                <NativeSelectControl
                                    label={__('Select Tag', 'marks')}
                                    value={headingTag}
                                    onChange={value => setAttributes({ headingTag: value })}
                                    options={[
                                        { label: __('H1', 'marks'), value: 'h1' },
                                        { label: __('H2', 'marks'), value: 'h2' },
                                        { label: __('H3', 'marks'), value: 'h3' },
                                        { label: __('H4', 'marks'), value: 'h4' },
                                        { label: __('H5', 'marks'), value: 'h5' },
                                        { label: __('H6', 'marks'), value: 'h6' },
                                        { label: __('Paragraph', 'marks'), value: 'p' },
                                        { label: __('Div', 'marks'), value: 'div' }
                                    ]}
                                />
                                <NativeTextControl
                                    label={__('Title Text', 'marks')}
                                    value={heading}
                                    onChange={value => setAttributes({ heading: value })}
                                    placeholder={__('List title...', 'marks')}
                                />
                            </>
                        )}
                    </PanelBody>
            ...
            <div {...blockProps}>
                <div className="marks-icon-block-wrapper">
                    <div
