...
if ( ! function_exists( 'marks_enqueue_heading_arrow_editor_assets' ) ) :
	/**
	 ...
	 */
	function marks_enqueue_heading_arrow_editor_assets() {
...
add_action( 'enqueue_block_editor_assets', 'marks_enqueue_heading_arrow_editor_assets' );


if ( ! function_exists( 'marks_render_heading_arrow' ) ) :
	/**
	 ...
	 * @return string Modified block HTML.
	 */
	function marks_render_heading_arrow( $block_content, $block ) {
...
add_filter( 'render_block', 'marks_render_heading_arrow', 10, 2 );
...
if ( ! function_exists( 'marks_enqueue_group_overlay_ellipse_editor_assets' ) ) :
	/**
	 * Enqueues the group-overlay-ellipse extension script and editor stylesheet.
	 */
	function marks_enqueue_group_overlay_ellipse_editor_assets() {
		...
		wp_enqueue_script(
			'marks-group-overlay-ellipse-extension',
...
add_action( 'enqueue_block_editor_assets', 'marks_enqueue_group_overlay_ellipse_editor_assets' );


if ( ! function_exists( 'marks_enqueue_group_overlay_ellipse_frontend_assets' ) ) :
	/**
	 * Enqueues the group-overlay-ellipse frontend stylesheet.
	 */
	function marks_enqueue_group_overlay_ellipse_frontend_assets() {
		$asset_file = get_theme_file_path( 'build/extensions/group-overlay-ellipse/index.asset.php' );
		...
		wp_enqueue_style(
			'marks-group-overlay-ellipse-extension-style',
...
add_action( 'enqueue_block_assets', 'marks_enqueue_group_overlay_ellipse_frontend_assets' );


if ( ! function_exists( 'marks_render_group_overlay_ellipse' ) ) :
	/**
	 ...
	 * @return string Modified block HTML.
	 */
	function marks_render_group_overlay_ellipse( $block_content, $block ) {
		$supported = array( 'core/group' );
...
add_filter( 'render_block', 'marks_render_group_overlay_ellipse', 10, 2 );


// =============================================================================
// Group – Floating Border Extension
// =============================================================================

if ( ! function_exists( 'marks_enqueue_group_floating_border_editor_assets' ) ) :
	/**
	 * Enqueues the group-floating-border extension script and editor stylesheet.
	 */
	function marks_enqueue_group_floating_border_editor_assets() {
		$asset_file = get_theme_file_path( 'build/extensions/group-floating-border/index.asset.php' );
		...
		wp_enqueue_script(
			'marks-group-floating-border-extension',
			get_theme_file_uri( 'build/extensions/group-floating-border/index.js' ),
			$assets['dependencies'],
			$assets['version'],
			true
		);

		$editor_css = get_theme_file_path( 'build/extensions/group-floating-border/index.css' );
		if ( file_exists( $editor_css ) ) {
			wp_enqueue_style(
				'marks-group-floating-border-extension',
				get_theme_file_uri( 'build/extensions/group-floating-border/index.css' ),
				array(),
				$assets['version']
			);
		}
...
add_action( 'enqueue_block_editor_assets', 'marks_enqueue_group_floating_border_editor_assets' );


if ( ! function_exists( 'marks_enqueue_group_floating_border_frontend_assets' ) ) :
	/**
	 * Enqueues the group-floating-border frontend stylesheet.
	 */
	function marks_enqueue_group_floating_border_frontend_assets() {
		$asset_file = get_theme_file_path( 'build/extensions/group-floating-border/index.asset.php' );
		$style_file = get_theme_file_path( 'build/extensions/group-floating-border/style-index.css' );
		...
		wp_enqueue_style(
			'marks-group-floating-border-extension-style',
			get_theme_file_uri( 'build/extensions/group-floating-border/style-index.css' ),
			array(),
			$assets['version']
		);
	}
endif;
add_action( 'enqueue_block_assets', 'marks_enqueue_group_floating_border_frontend_assets' );


if ( ! function_exists( 'marks_render_group_floating_border' ) ) :
	/**
	 * Injects floating border CSS variables and classes into supported blocks on the frontend.
	 *
	 * @param string $block_content The rendered block HTML.
	 * @param array  $block         The block data including name and attributes.
	 * @return string Modified block HTML.
	 */
	function marks_render_group_floating_border( $block_content, $block ) {
		...
		$border1_enabled = ! empty( $attrs['floatingBorder1Enabled'] );
		$border2_enabled = ! empty( $attrs['floatingBorder2Enabled'] );

		if ( ! $border1_enabled && ! $border2_enabled ) {
			return $block_content;
		}

		$new_classes = array();
		$css_vars    = array();
		$fields      = array( 'Width', 'Height', 'Color', 'Top', 'Right', 'Bottom', 'Left' );

		if ( $border1_enabled ) {
			$new_classes[] = 'has-floating-border-1';
			foreach ( $fields as $field ) {
				$val = $attrs[ 'floatingBorder1' . $field ] ?? '';
				if ( '' !== $val ) {
					$css_vars[] = '--fb1-' . strtolower( $field ) . ':' . esc_attr( $val );
				}
			}
		}

		if ( $border2_enabled ) {
			$new_classes[] = 'has-floating-border-2';
			foreach ( $fields as $field ) {
				$val = $attrs[ 'floatingBorder2' . $field ] ?? '';
				if ( '' !== $val ) {
					$css_vars[] = '--fb2-' . strtolower( $field ) . ':' . esc_attr( $val );
				}
			}
		}
...
add_filter( 'render_block', 'marks_render_group_floating_border', 10, 2 );
